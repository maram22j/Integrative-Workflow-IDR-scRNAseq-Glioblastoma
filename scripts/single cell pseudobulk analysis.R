...
head(data)
#loading metadata------------------------------
metadata<-read.csv("C:/Users/maram/Desktop/intital 1/Internships/Centre Biotechnologique de Sfax/codes/single cell/metadata/GSE131928_single_cells_tumor_name_and_adult_or_peidatric.csv",header = T,sep =";" )
...
ggsave("single cell rna seq analysis/plots/qc_violin_inspect_only.png", width = 8, height = 4)
plot2 <- FeatureScatter(object, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
plot2
ggsave("single cell rna seq analysis/plots/scatter_plot_of_QC.png", width = 8, height = 4)
# filtration
object <- subset( object,subset = nFeature_RNA > 1000 & nFeature_RNA < 10000)
VlnPlot(object, features = c("nFeature_RNA", "nCount_RNA"), ncol = 2)
ggsave("single cell rna seq analysis/plots/Volcano_plot_after_filtration.png", width = 8, height = 4)
#Cell filtration:-----------------------------------------------
...
object$cna_signal <- cna_results$cna_signal[
  match(colnames(object), cna_results$cell)
]
object$cna_correlation <- cna_results$cna_correlation[
  match(colnames(object), cna_results$cell)
]
...
saveRDS(object_malignant, "single cell rna seq analysis/malignant_cells_only.rds")
...
print(plot3)
print(plot4)
ggsave("single cell rna seq analysis/plots/variable_features_annotated.png", width = 8, height = 4)
#scaling:------------------------------------------------------------
...
ggsave("single cell rna seq analysis/plots/elbow_plot.png", width = 6, height = 4)
n_pcs <- 10
# Clustering----------------------------------
...
for(i in seq_along(plots)){
  ggsave(
    paste0("single cell rna seq analysis/PCA_Results/",names(plots)[i],".png"),
    plots[[i]],
    width=7,
    height=6,
    dpi=300
...
pdf("single cell rna seq analysis/PCA_Results/Loadings_PC1_PC5.pdf", width = 12, height = 10)
VizDimLoadings(object_malignant, dims = 1:5, reduction = "pca",nfeatures = 10)
dev.off()

pdf("single cell rna seq analysis/PCA_Results/Loadings_PC6_PC10.pdf", width = 12, height = 10)
VizDimLoadings(object_malignant, dims = 6:10, reduction = "pca",nfeatures=10)
dev.off()
pdf("single cell rna seq analysis/PCA_Results/Loadings_PC1_PC10.pdf", width = 12, height = 10)
...
pdf(
  "single cell rna seq analysis/PCA_Results/08_PCA_Heatmap_PC1_PC10.pdf",
  width=12,
  height=12
)
...
write.csv(
  variance,
  "single cell rna seq analysis/PCA_Results/PCA_Variance_Explained.csv",
  row.names=FALSE
)
# Variance Plot
##############################
...
ggsave(
  "single cell rna seq analysis/PCA_Results/09_Variance_Explained.png",
  p,
  width=7,
  height=5,
  dpi=300
...
write.csv(
  coords,
  "single cell rna seq analysis/PCA_Results/PCA_Cell_Coordinates.csv"
)
# PCA Loadings
##############################
...
write.csv(
  loadings,
  "single cell rna seq analysis/PCA_Results/PCA_Gene_Loadings.csv"
)
# Top Genes per PC
##############################
...
write.csv(
  top.genes,
  "single cell rna seq analysis/PCA_Results/Top_Genes_Per_PC.csv",
  row.names=FALSE
)
# visualization of UMAP and tSNE
#save UMAP
...
DimPlot(object_malignant, reduction = "umap", group.by = "seurat_clusters", label = TRUE) +
  ggtitle("Clusters")
ggsave("single cell rna seq analysis/plots/umap_by_clusters_annotation.png", width = 6, height = 5)
DimPlot(object_malignant, reduction = "umap", group.by = "tumour.name") +
  ggtitle("By tumor of origin")
ggsave("single cell rna seq analysis/plots/umap_by_tumour_annotation.png", width = 6, height = 5)
DimPlot(object_malignant, reduction = "umap", group.by = "adult.pediatric") +
  ggtitle("Adult vs Pediatric")
ggsave("single cell rna seq analysis/plots/umap_by_age_annotation.png", width = 6, height = 5)
#tSNE plot
...
ggsave("single cell rna seq analysis/plots/tsne_by_classified_by_tumour.png", width = 6, height = 5)
DimPlot(object_malignant,reduction = "tsne",group.by = "seurat_clusters",label = T)+
  ggtitle("Grouped by clusters")
ggsave("single cell rna seq analysis/plots/tsne_classified_by_clusters.png", width = 6, height = 5)

#Save processed object for downstream IDR-integration work ------------------------
saveRDS(object_malignant, file = "object_malignant.rds")
expr_normalized <- GetAssayData(object_malignant, layer = "data")
write.csv(as.matrix(expr_normalized), "single cell rna seq analysis/GSE131928_SS2_expression_normalized.csv")
cell_metadata <- object_malignant@meta.data
write.csv(cell_metadata, "metadata.csv")
save.image("single cell rna seq analysis/Single_cell_RNA_seq.RData")
